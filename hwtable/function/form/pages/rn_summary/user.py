...
from function.form.operation.table.table_user import TableUserWidget, CustomTableWidget  # 确保导入正确
...
class RnSummaryUserWindow(QMainWindow):
    def __init__(self, table=None):
        ...
        # 创建并添加表格
        if table is None:
            self.table_widget = TableUserWidget.create_table_widget()
        else:
            self.table_widget = TableUserWidget(table)

        # 设置列标题
        self.set_column_headers(self.table_widget.get_table_user())

        # 原始数据表格
        self.original_table = CustomTableWidget(self.table_widget.get_table_user().rowCount(), self.table_widget.get_table_user().columnCount())
        self.copy_table(self.table_widget.get_table_user(), self.original_table)

        # 副本表格，用于显示筛选结果
        self.filtered_table_widget = CustomTableWidget(self.original_table.rowCount(), self.original_table.columnCount())
...
if __name__ == "__main__":
    ...
    # 创建一个示例表格
    example_table = CustomTableWidget(3, 7)
    for row in range(3):
        for col in range(7):
            example_table.setItem(row, col, QTableWidgetItem(str((row + 1) * (col + 1))))

    window = RnSummaryUserWindow(example_table)
    window.show()

...
from function.form.operation.table.table_user import TableUserWidget  # 确保导入正确
from function.database.database import MongoDBHandler

class ChangeRecordsUserWindow(QMainWindow):
    def __init__(self, table=None):
        ...
        # 创建并添加表格
        if table is None:
            self.table_widget = TableUserWidget.create_table_user_widget()
        else:
    ...
    def add_summary_row(self):
        table = self.table_widget.get_table_user()
...
if __name__ == "__main__":
    ...
    window = ChangeRecordsUserWindow()
    window.show()
